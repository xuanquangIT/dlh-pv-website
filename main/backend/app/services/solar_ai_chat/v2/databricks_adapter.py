"""Solar Chat v2 — adapter that wires the v1 BaseRepository's Databricks
connection into the v2 primitives.

Why an adapter: BaseRepository owns connection management. Rather than
duplicating that logic in v2, we wrap an existing repository instance and
expose its `_execute_query` as a `Callable[[str], list[dict]]` that the
primitives accept.
"""
...
def make_sql_executor(
    ...
    """Return a callable suitable for v2 primitives.execute_sql.
